...
│   │           └── model_info_full.pkl
│   │   # Note: the "Quick Predict" simplified model (train_simplified_model.py) was
│   │   # removed 2026-07-14 — see §5a "Retired: Simplified Model" for why.
│   │
│   ├── ml_service/
...
└── frontend/
    ...
    └── src/
        ...
        │   ├── About.jsx
        │   ├── ContactPage.jsx
        │   ├── CropRecommendation.jsx  # ML: full-analysis predict form + results
...
Crop Recommendation runs on a single mode — **Full Analysis**: 7 numeric + 4 categorical = 11 input params, scored by an RF + XGBoost ensemble at ~90% Top-1 / ~99% Top-3 accuracy.

A second, lighter "Quick Predict" mode existed from 2026-06 through 2026-07-14 and was removed after it proved too inaccurate to keep in production. See **§5a** below for what it was and why it was retired.
...
### 5a. Retired: Simplified Model ("Quick Predict")

**Status:** removed 2026-07-14. `train_simplified_model.py` and its generated artifacts (`crop_model_simple.pkl`, `label_encoder_simple.pkl`, `model_info_simple.pkl`) were deleted; the `/predict/simple` endpoint, `SimplifiedModeRequest` schema, and the frontend mode toggle were removed from the codebase. This section is kept as a record of what existed and why it was cut.

**Why it was created**

Most smallholder farmers don't have a soil-test kit and can't supply N, P, K, or pH values on the spot — those four fields in Full Analysis require either a lab test or an extension officer visit. Quick Predict was built as a low-friction alternative: give it just the four categorical fields the farmer already knows off the top of their head — `Soil_Type`, `Agro_Zone`, `Irrigation`, `Season` (plus an optional `District`) — and get an instant recommendation with no soil test needed.

It went through two iterations:
- **v1 — categorical-only:** one-hot-encoded `Soil_Type` / `Agro_Zone` / `Irrigation` / `Season` (optionally `District`), Random Forest with `GridSearchCV`.
- **v2 — weather-enhanced:** added three numeric features (`Temperature`, `Rainfall`, `Humidity`) auto-filled from the live Open-Meteo weather API the moment the farmer picked a district, plus one engineered ratio (`Rainfall_Temp_Ratio`), on the theory that free, auto-fetched climate data would recover some of the accuracy lost by dropping soil chemistry.

**Why it failed**

| Model | Input | Accuracy (Top-1) |
|---|---|---|
| Full Analysis | 7 numeric + 4 categorical (11 params) | **90.7%** |
| Quick Predict v2 (weather-enhanced) | 4 categorical + 3 weather numerics | **65.3%** |

A ~25-point accuracy gap persisted no matter how the categorical/weather model was tuned (`GridSearchCV` swept `n_estimators`, `max_depth`, `min_samples_split/leaf`, `max_features` — see the deleted training script for the full grid). The root cause wasn't hyperparameters, it was feature signal: soil nutrient levels (N/P/K) and pH are the dominant predictors of what a crop needs to thrive, and no combination of soil-*type*, zone, irrigation method, season, or ambient weather fully substitutes for actually measuring them. Categorical features tell you what *kind* of place a field is; N/P/K and pH tell you what condition that specific field is actually in right now — and that distinction is what crop suitability hinges on.

A 65% top-1 model means roughly one in three farmers would have been steered toward a suboptimal or wrong crop for their season — an unacceptable failure rate for a recommendation with real planting-season consequences. Presenting that alongside a 90.7%-accurate mode as an equally valid "quick" choice risked farmers defaulting to the faster, less accurate option without understanding the trade-off. The team decided one trustworthy mode beats two modes where one quietly underperforms, and retired Quick Predict rather than keep shipping it.

**Where this leaves the product:** Full Analysis remains the only prediction path. To lower the friction that motivated Quick Predict in the first place, the weather auto-fill (`Temperature`, `Rainfall`, `Humidity` populated from `/weather` once a district is selected) was kept in Full Analysis — farmers only need to manually supply N, P, K, and pH, ideally from a soil test or extension-officer visit.
...
1. Loads 3 pickle files from `models/` (full model + label encoder + model info)
...
**Inference path (no pandas):** Input dicts are converted directly to NumPy arrays via `_build_full_input()`. One-hot encoding is replicated by building a `{col_val: 1}` dict and reading columns in saved feature order.
...
```json
{
  "status": "ok",
  "version": "5.3.0",
  "full_model": { "loaded": true, "accuracy": 0.9070, "type": "VotingClassifier(RF+XGBoost)", "calibration_T": 0.5997 },
  "crop_info_crops": 41,
  "cache_entries": 0
}
```
...
- Two-mode form: Quick Predict + Full Analysis. (Quick Predict later removed in v9.0 — see §5a.)
...
### v9.0 — Simplified model retired (2026-07-14)

- **"Quick Predict" mode removed:** `train_simplified_model.py`, its three generated artifacts, the `/predict/simple` endpoint, `SimplifiedModeRequest` schema, and the frontend mode toggle were all deleted. Full details and rationale in **§5a "Retired: Simplified Model"**.
- **Why:** at 65.3% Top-1 accuracy (vs. 90.7% for Full Analysis), the weather-enhanced categorical-only model produced a wrong-crop recommendation roughly 1 in 3 times — too unreliable to keep offering as a peer option to Full Analysis.
